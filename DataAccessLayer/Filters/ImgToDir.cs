...
namespace DataAccessLayer.Filters
{
    ...
    {
        public string CopyFile(FileUser imageFile, List<string> permExtensions, ClinicaDbContext _context, int userId)
        {
            if (imageFile != null)
            {
        ...
        public string CopyPdf(IFormFile imageFile, List<string> permExtensions, string uploadDirectory, ClinicaDbContext _context, int userId, int apponitID)
        {
            if (imageFile != null)
            {
                ...
                var pdfFile = _context.Appointments.FirstOrDefault(x => x.AppointId == apponitID);
